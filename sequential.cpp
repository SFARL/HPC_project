...
// int main_standard(int argc, char **argv)
{
    ...
    float flop = 6 * flop_cnt * n;
    cout << "is safe cnt: " << flop_cnt << endl;
    float gflop = flop / (std::chrono::duration_cast<std::chrono::nanoseconds>(t_end - t_start).count());
    cout << "flops = " << gflop << " Gflop/s" << endl;
    cout << "percentage of peak " << gflop / 2.894 / 4 * 100 << "%" << endl;
    cout << "float rate per memory access " << 2 << endl;
    free(grid);
    return 0;
}
